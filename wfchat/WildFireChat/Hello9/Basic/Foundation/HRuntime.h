 ...
 */
CG_INLINE BOOL
HExchangeImplementationsInTwoClasses(Class _fromClass, SEL _originSelector, Class _toClass, SEL _newSelector) {
...
CG_INLINE BOOL
HExchangeImplementations(Class _class, SEL _originSelector, SEL _newSelector) {
    return HExchangeImplementationsInTwoClasses(_class, _originSelector, _class, _newSelector);
}
...
/// @note 不包括父类的对象方法
CG_INLINE BOOL
HClassContainSelector(Class aClass, SEL selector) {
    unsigned int methodCount;
    Method *methodList = class_copyMethodList(aClass, &methodCount);
    NSString *methodName = NSStringFromSelector(selector);
    for (int i = 0; i < methodCount; i ++) {
        NSString *currMethodName = NSStringFromSelector(method_getName(methodList[i]));
        if ([currMethodName isEqualToString:methodName]) {
            free(methodList);
            return YES;
        }
    }
